...
func QueryClubTagByClubId(clubId string) (club *model.ClubsTags, err error) {
...
func QueryClubIdByTagIds(tagIds []int64, clubId string) (clubTagIds []string, err error) {
	if err = global.DB.Model(&model.ClubsTags{}).
		Where("club_id = ? and tag_id in ? and state = 0", clubId, tagIds).
		Pluck("tag_id", &clubTagIds).Error; err != nil {
		global.Lg.Error("查询俱乐部标签失败", zap.Error(err))
		return nil, err
	}
	return clubTagIds, nil
}
